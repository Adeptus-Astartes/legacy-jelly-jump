...
public class Waves: MonoBehaviour 
{
	...
	void LateUpdate () {
		...
		for (int i=0;i<vertices.Length;i++) {
			...
			//vertex.y += Mathf.Sin(Time.unscaledTime * speed + baseHeight[i].x * waveLength + baseHeight[i].y * waveLength) * waveHeight;
			vertex.y += Mathf.Sin(Mathf.Cos(Random.value * 1.0f) * randomHeight * Mathf.Cos (Time.unscaledTime * randomSpeed * Mathf.Sin(Random.value * 1.0f)));
			//vertex.y += Mathf.PerlinNoise(baseHeight[i].x + Mathf.Cos(Time.unscaledTime * 0.1f) + noiseWalk, baseHeight[i].y + Mathf.Sin(Time.unscaledTime * 0.1f)) * noiseStrength;
			vertices[i] = vertex;
		}
